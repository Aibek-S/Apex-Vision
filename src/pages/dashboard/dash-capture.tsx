...
const modeOptions: { value: CaptureMode; label: string; description: string }[] =
  [
    {
      value: "manual",
      label: "Manual Upload",
    ...
    },
    {
      ...
      description: "Подключитесь к устройству и дождитесь завершения сканирования",
    },
  ];
...
export default function DashCapture() {
  ...
  const artifactName =
    (location.state as { artifactName?: string } | undefined)?.artifactName ||
    "Артефакт";
  ...
  const manualModeActive =
    session?.capture_mode === "manual" || (!session && selectedMode === "manual");
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="rounded-2xl border border-black/5 bg-white/60 px-4 py-3 text-sm">
          <p className="font-semibold text-textDark">{sessionStatus}</p>
          {session && (
      ...
      <section className="grid gap-6 md:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {modeOptions.map((mode) => {
              ...
              return (
                ...
                >
                  <p className="font-bold text-textDark">{mode.label}</p>
                  <p className="text-sm text-secondary mt-1">{mode.description}</p>
                  {mode.value === "device" && (
                    <p className="mt-3 text-xs text-secondary/70">
                      Заглушка: UI готов, интеграция с ESP в разработке.
                    </p>
                  )}
                </button>
              );
        ...
        <div className="space-y-4">
                ...
                {images.map((image) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-secondary/80">
                      {(image.file_size / (1024 * 1024)).toFixed(2)} МБ
                    </p>
                  </li>
                ))}
              </ul>
          ...
          <div className="rounded-3xl border border-dashed border-primary/30 bg-primary/5 p-5 text-sm text-secondary space-y-2">
            ...
            <p>
              Этот блок служит заглушкой: здесь появится статус онлайн-устройства,
              лог загрузок и кнопка подтверждения завершения сканирования.
            </p>
          </div>
