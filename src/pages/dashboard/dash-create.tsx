...
export default function DashCreate() {
    ...
    const [modelCodeDraft, setModelCodeDraft] = useState<string>("");
    ...
    const applyAiField = (
        ...
        setForm((prev) => {
            switch (field) {
                ...
                case "bibliography":
                    return { ...prev, bibliography: value ? String(value) : "" };
                case "report_author":
                    return { ...prev, reportAuthor: value ? String(value) : "" };
    ...
    const persistModelCode = (value: string) => {
        if (!projectId) return;
        if (value) {
            window.localStorage.setItem(`artifactModelCode:${projectId}`, value);
        } else {
            window.localStorage.removeItem(`artifactModelCode:${projectId}`);
        }
    };

    const applyModelCode = () => {
        const trimmed = modelCodeDraft.trim();
        setModelCode(trimmed);
        setModelCodeDraft(trimmed);
        persistModelCode(trimmed);
    };
    ...
    const fetchLatestModel = async () => {
        ...
        try {
            ...
            const requestUrl = trimmedCode
                ? `${WATCHER_API_URL}?code=${encodeURIComponent(trimmedCode)}`
                : WATCHER_API_URL;
            ...
            if (response.status === 404) {
                ...
                setModelReady(false);
                setModelError(
                    "3D-модель не найдена. Проверьте папку результатов."
                );
                return;
            }
    ...
    const ensureArtifact = async () => {
        if (!user) {
            throw new Error("Сначала войдите в систему, чтобы сохранить артефакт.");
        }
    ...
    const handlePhotoUpload = async (file: File | null) => {
        ...
        try {
            ...
            setForm((prev) => ({ ...prev, photoUrl: uploadResult.storagePath }));
    ...
    const captureAiScreenshots = async () => {
        ...
        const viewer =
            modelType === "obj" ? objViewerRef.current : modelViewerRef.current;
        ...
        const loadData = async () => {
            ...
            if (measurement) {
                ...
                setForm((prev) => ({
                    ...
                    restorationDetails:
                        measurement.restoration_details ?? "",
                }));
            }
    ...
    useEffect(() => {
        ...
        const stored = window.localStorage.getItem(
            `artifactModelCode:${projectId}`
        );
        if (stored) {
            setModelCode(stored);
            setModelCodeDraft(stored);
        } else {
            setModelCode("");
            setModelCodeDraft("");
        }
            ...
            <header className="space-y-2">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <p className="text-xs font-black uppercase tracking-[0.3em] text-primary/70">
                        {isEditMode ? "Редактирование отчета" : "Создание отчета"}
                    </p>
                    ...
                    <section className="rounded-[2rem] border border-black/10 bg-white/80 p-6 space-y-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <Button
                                ...
                                disabled={aiLoading || aiPreparing || !modelReady}
                            >
                                {aiLoading
                                    ? "Отправка..."
                                    : aiPreparing
                        ...
                        {aiSuggestions && (
                            <div className="grid gap-4 md:grid-cols-2 text-sm text-secondary">
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("dating", aiSuggestions.dating);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            dating: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("dating", aiBaseline.dating);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            dating: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("material", aiSuggestions.material);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            material: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("material", aiBaseline.material);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            material: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("technique", aiSuggestions.technique);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            technique: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("technique", aiBaseline.technique);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            technique: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("quantity", aiSuggestions.quantity);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            quantity: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("quantity", aiBaseline.quantity);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            quantity: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("subject", aiSuggestions.subject);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            subject: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("subject", aiBaseline.subject);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            subject: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    onApply={() => {
                                        applyAiField("legend", aiSuggestions.legend);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            legend: true,
                                        }));
                                    ...
                                    onKeep={() => {
                                        applyAiField("legend", aiBaseline.legend);
                                        setAiApplied((prev) => ({
                                            ...prev,
                                            legend: false,
                                        }));
                                ...
                                <AiDetail
                                    ...
                                    originalValue={aiBaseline.condition_description}
                                    aiValue={aiSuggestions.condition_description}
                                ...
                                <AiDetail
                                    ...
                                    originalValue={aiBaseline.restoration_details}
                        ...
                        {!aiSuggestions && (
                            <div className="rounded-2xl border border-black/10 bg-black/[0.02] px-4 py-3 text-xs text-secondary/80">
                                Запустите классификацию, чтобы получить подсказки
                                APEX-AI.
                            </div>
                        )}
                    </section>
                    ...
                    <section className="rounded-[2rem] border border-black/10 bg-white/80 p-6 space-y-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={fetchLatestModel}
                                disabled={modelLoading}
                            >
                                {modelLoading ? "Загрузка..." : "Обновить модель"}
                            </Button>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-secondary/80">
                            ...
                            {modelCode.trim() && (
                                <span>Код модели: {modelCode.trim()}</span>
                            )}
                        </div>
                        ...
                        <div className="rounded-2xl border border-black/10 bg-white/70 p-4 space-y-3">
                            <div className="space-y-1 text-sm text-secondary">
                                <span className="font-medium text-textDark">
                                    Код модели (опционально)
                                </span>
                                <input
                                    type="text"
                                    value={modelCodeDraft}
                                    onChange={(event) =>
                                        setModelCodeDraft(event.target.value)
                                    }
                                    placeholder="Оставьте пустым для последней модели"
                            ...
                            <p className="text-xs text-secondary/70">
                                Если код не указан, берется последняя найденная
                                модель из watcher.
                            </p>
                        </div>
